...
def evaluate(options: Box, checkpoint_path: str, log_path: str, test_loader: DataLoader, num_threshold: int = 9) -> None:
    # Preliminary setups
    ...
    with torch.no_grad():
        for threshold in torch.arange(0, 1, round(1 / num_threshold, 1)):
            if threshold == 0:
